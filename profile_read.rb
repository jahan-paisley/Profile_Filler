...
class Profile_Reader
  ...
  def initialize
    @email_params = {:username => 'sample_email_username', :password => 'sample_email_password', :host => 'mail.example.com', :senders => ['shokri']}
    @sql_params = {:username => 'sample_sqlserver_user', :password => 'sample_sqlserver_password', :host => '1.1.1.1'}
  ...
  def email_read
    ...
    if @email_params[:senders].length >1
      ...
      senders = ["FROM"]+@email_params[:senders]
    end
  ...
  def set_dates
    ...
    sstart = (last_date || Time.now.to_datetime.prev_day(7)).strftime(format)
